...
<div class="bg-white bg-opacity-80 p-6 rounded-lg shadow-lg">
    ...
    <%
        ...
        if (service == null) {
            out.println("<p class='text-red-500'>Service not found!</p>");
        } else {
    %>
    <form action="service" method="post" class="space-y-4">
        ...
        <div>
            <label>ID (e.g., SE01):</label>
            <input type="text" name="id" value="<%= service.getId() %>" required class="border p-2 w-full" pattern="SE[0-9]{2}" title="ID must be in format SE followed by two digits (e.g., SE01)">
        </div>
        <div>
            <label>Name:</label>
            <input type="text" name="name" value="<%= service.getName() %>" required class="border p-2 w-full">
        </div>
        <div>
            <label>Duration (e.g., 45-90):</label>
            <input type="text" name="duration" value="<%= service.getDuration() %>" required class="border p-2 w-full" pattern="\d+-\d+" title="Duration must be in format number-number (e.g., 45-90)">
        </div>
        <div>
            <label>Price (LKR):</label>
            <input type="number" step="0.01" name="price" value="<%= String.format("%.2f", service.getPrice()) %>" required class="border p-2 w-full">
        </div>
        <div>
            <label>Description:</label>
            <textarea name="description" class="border p-2 w-full" rows="4"><%= service.getDescription() %></textarea>
        </div>
        <div>
            <label>Type:</label>
        ...
        <button type="submit" class="bg-purple-500 text-white px-4 py-2 rounded hover:bg-purple-600">Update Service</button>
    </form>
    <% } %>
